...
let MailService = MailService_1 = class MailService {
    ...
    async notifyDailyReport(report) {
      ...
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto">
        ...
        this.send(recipients, `Daily Report ${report.status === 'SUBMITTED' ? 'Submitted' : 'Created'} — ${report.user.name} (${dateStr})`, html);
    }
    async notifyVisit(visit) {
        const recipients = await this.getSuperAdminEmails();
        if (!recipients.length)
            return;
        const target = visit.visitType === 'DOCTOR'
