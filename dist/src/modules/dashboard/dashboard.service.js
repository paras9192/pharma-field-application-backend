...
let DashboardService = class DashboardService {
    ...
    async getAdminDashboard(date) {
        const targetDate = date ? (0, dayjs_1.default)(date) : (0, dayjs_1.default)();
        const start = targetDate.startOf('day').toDate();
        const end = targetDate.endOf('day').toDate();
        const [totalEmployees, activeEmployees, presentToday, totalVisitsToday, doctorVisitsToday, chemistVisitsToday, pendingFollowUps, reportsSubmittedToday, totalDoctors, totalChemists,] = await Promise.all([
        ...
        const topPerformerDetails = await Promise.all(topPerformers.map(async (p) => {
    ...
    async getEmployeeDashboard(userId, date) {
        const targetDate = date ? (0, dayjs_1.default)(date) : (0, dayjs_1.default)();
        const start = targetDate.startOf('day').toDate();
        const end = targetDate.endOf('day').toDate();
        const [todayAttendance, todayVisits, doctorVisitsToday, chemistVisitsToday, pendingFollowUps, totalVisitsMonth, todayReport,] = await Promise.all([
    ...
    async getEmployeePerformance(from, to) {
        const start = from ? (0, dayjs_1.default)(from).startOf('day').toDate() : (0, dayjs_1.default)().startOf('month').toDate();
        const end = to ? (0, dayjs_1.default)(to).endOf('day').toDate() : (0, dayjs_1.default)().endOf('month').toDate();
        const employees = await this.prisma.user.findMany({
            where: { isActive: true, role: { name: { in: ['MR', 'SALES_PERSON'] } } },
            select: { id: true, name: true, employeeCode: true, role: true },
        });
        const performance = await Promise.all(employees.map(async (emp) => {
            const [totalVisits, doctorVisits, chemistVisits, daysPresent, reportsSubmitted] = await Promise.all([
    ...
    async getSuperAdminDashboard(date) {
        ...
        const [billAggregate, overdueAggregate, paymentAggregate, totalChemists, totalDoctors, totalEmployees, presentToday, visitsToday, billsToday, billsThisMonth, pendingFollowUps,] = await Promise.all([
        ...
        const salespersonLeaderboard = await Promise.all(spCollectionsRaw.map(async (s, idx) => {
            const user = await this.prisma.user.findUnique({
        ...
        const mrLeaderboard = await Promise.all(mrVisitsRaw.map(async (v, idx) => {
            const user = await this.prisma.user.findUnique({
                where: { id: v.userId },
                select: { id: true, name: true, employeeCode: true, role: true },
            });
    ...
    async getPaymentDashboard(currentUser, filters = {}) {
        ...
        if (filters.month && filters.year) {
            const m = (0, dayjs_1.default)().year(filters.year).month(filters.month - 1);
            ...
            if (filters.from)
                periodStart = (0, dayjs_1.default)(filters.from).startOf('day').toDate();
            if (filters.to)
                periodEnd = (0, dayjs_1.default)(filters.to).endOf('day').toDate();
            const fromLabel = filters.from ? (0, dayjs_1.default)(filters.from).format('D MMM YYYY') : '—';
            const toLabel = filters.to ? (0, dayjs_1.default)(filters.to).format('D MMM YYYY') : '—';
        ...
        if (filters.status)
            billBaseWhere.status = filters.status;
        const billWhere = { ...billBaseWhere };
        if (periodStart || periodEnd) {
            billWhere.createdAt = {};
            if (periodStart)
                billWhere.createdAt.gte = periodStart;
            if (periodEnd)
                billWhere.createdAt.lte = periodEnd;
        }
        const paymentWhere = {};
        if (currentUser.role.name === role_enum_1.Role.SALES_PERSON) {
        ...
        if (periodStart || periodEnd) {
            ...
            if (periodStart)
                paymentWhere.collectedAt.gte = periodStart;
            if (periodEnd)
                paymentWhere.collectedAt.lte = periodEnd;
        }
        if (filters.chemistId && currentUser.role.name !== role_enum_1.Role.SALES_PERSON) {
        ...
        const agingBase = { ...billBaseWhere, status: { not: 'PAID' } };
        ...
        const [billTrend12Raw, paymentTrend12Raw] = await Promise.all([
            ...
            this.prisma.payment.findMany({
                where: {
                    ...(currentUser.role.name === role_enum_1.Role.SALES_PERSON ? { collectedById: currentUser.id } : {}),
                    ...(filters.chemistId && currentUser.role.name !== role_enum_1.Role.SALES_PERSON ? { bill: { chemistId: filters.chemistId } } : {}),
                    ...(filters.collectedById && currentUser.role.name !== role_enum_1.Role.SALES_PERSON ? { collectedById: filters.collectedById } : {}),
        ...
        let salespersonRanking = [];
        if (currentUser.role.name === role_enum_1.Role.SUPER_ADMIN || currentUser.role.name === role_enum_1.Role.ADMIN) {
            ...
            salespersonRanking = await Promise.all(spRaw.map(async (s, idx) => {
                const user = await this.prisma.user.findUnique({
                    where: { id: s.collectedById },
                    select: { id: true, name: true, employeeCode: true },
                });
        ...
        return {
            period: {
                label: periodLabel,
                from: periodStart ? (0, dayjs_1.default)(periodStart).format('YYYY-MM-DD') : null,
                to: periodEnd ? (0, dayjs_1.default)(periodEnd).format('YYYY-MM-DD') : null,
            ...
            upcomingCollections: upcomingBills.map((b) => ({
                ...
                daysUntilDue: (0, dayjs_1.default)(b.dueDate).diff(today, 'day'),
            })),
            highRiskAccounts: highRiskBills.map((b) => ({
                ...b,
                dueAmount: Number(b.dueAmount),
                daysOverdue: today.diff((0, dayjs_1.default)(b.dueDate), 'day'),
            })),
        };
    }
    async getSalesPersonDashboard(userId, date) {
        const today = date ? (0, dayjs_1.default)(date) : (0, dayjs_1.default)();
        const todayStart = today.startOf('day').toDate();
        const todayEnd = today.endOf('day').toDate();
        const monthStart = today.startOf('month').toDate();
        ...
        const [todayAttendance, todayCollectedAgg, todayVisits, pendingBillsCount, overdueCount, pendingFollowUps, monthlyCollectedAgg, monthlyBillAgg, monthlyBillsCount,] = await Promise.all([
        ...
        return {
            ...
            collectionTasks: collectionTasks.map((b) => {
                const daysUntilDue = (0, dayjs_1.default)(b.dueDate).diff(today, 'day');
                return {
                    ...b,
                    dueAmount: Number(b.dueAmount),
                    daysUntilDue,
    ...
    async getMRDashboard(userId, date) {
        const today = date ? (0, dayjs_1.default)(date) : (0, dayjs_1.default)();
        const todayStart = today.startOf('day').toDate();
        const todayEnd = today.endOf('day').toDate();
        const monthStart = today.startOf('month').toDate();
        const monthEnd = today.endOf('month').toDate();
        const [todayAttendance, todayVisitsCount, completedVisitsToday, pendingFollowUps, totalVisitsMonth, doctorVisitsMonth, chemistVisitsMonth, completedVisitsMonth, todayReport,] = await Promise.all([
            this.prisma.attendance.findUnique({ where: { userId_date: { userId, date: todayStart } } }),
            this.prisma.visit.count({ where: { userId, visitDate: { gte: todayStart, lte: todayEnd } } }),
    ...
    async getAlerts(currentUser) {
        ...
        else if (currentUser.role.name === role_enum_1.Role.MR) {
    ...
    buildMonthlyTrend(billRecords, paymentRecords, months) {
        ...
        for (const r of paymentRecords) {
            const key = (0, dayjs_1.default)(r.collectedAt).format('YYYY-MM');
            if (buckets[key]) {
        ...
        return Object.entries(buckets).map(([month, v]) => ({
            month,
            label: (0, dayjs_1.default)(month + '-01').format('MMM YYYY'),
            ...v,
            collectionRate: v.billValue > 0 ? Math.round((v.collected / v.billValue) * 100) : 0,
        }));
    }
    buildDailyTrend(records, dateField, amountField, days) {
        const today = (0, dayjs_1.default)();
        const buckets = {};
        ...
        for (const r of records) {
            const key = (0, dayjs_1.default)(r[dateField]).format('YYYY-MM-DD');
            if (buckets[key]) {
