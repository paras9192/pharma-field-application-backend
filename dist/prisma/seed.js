...
async function main() {
    ...
    const [superAdminRole, adminRole, salesPersonRole] = await Promise.all([
        prisma.role.findUnique({ where: { name: client_1.RoleName.SUPER_ADMIN } }),
        prisma.role.findUnique({ where: { name: client_1.RoleName.ADMIN } }),
        prisma.role.findUnique({ where: { name: client_1.RoleName.SALES_PERSON } }),
    ]);
    ...
    const makeNum = (n) => `ORD-20260601-${String(n).padStart(4, '0')}`;
    const orderDefs = [
        {
    ...
    for (const o of orderDefs) {
        ...
        orders.push(await prisma.order.create({
            data: {
                ...
                items: { create: items.map((i) => ({ ...i, amount: i.quantity * i.rate })) },
            },
        }));
    }
    ...
    for (const b of billDefs) {
        ...
        bills.push(await prisma.bill.create({ data: { ...b, paidAmount: 0, dueAmount: b.totalAmount, status: 'UNPAID' } }));
    }
    ...
    for (const p of [
        ...
        if (!exists)
            await prisma.payment.create({ data: { billId: bill1.id, collectedById: sp1.id, ...p } });
    }
